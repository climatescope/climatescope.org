...
function chart__installed_capacity(element_id) {
  ...
  _self.draw_chart = function() {
    ...
    chart_container.select(".x.axis")
      .attr("transform", "translate(0," + (height + 32) + ")")
      .call(xAxis);
      
    chart_container.select(".x.axis .label")
      .attr("x", width)
      .attr("y", -12)
      .text(meta_info['label-x']);
  
    chart_container.select(".y.axis")
      .attr("transform", "translate(-32,0)")
      .call(yAxis);
   
    chart_container.select(".y.axis .label")
      .attr("y", 20)
...
function chart__clean_energy_investments(element_id) {
  ...
  d3.json(url, function(error, DATA) {
    ...
    // Compute the y domain taking into account the stacked values.
    y.domain([0, d3.max(chart_data, function (c) { 
    ...
    svg.append("rect")
      ...
      .style("fill", "none")
      .style("pointer-events", "all")
      .on("mouseover", function() { focus.style("display", null); chart_tooltip.style("display", null); chart_container.classed({mouseover: true}); })
      .on("mouseout", function() { focus.style("display", "none"); chart_tooltip.style("display", "none"); chart_container.classed({mouseover: false}); })
      .on("mousemove", function() {
        // Define bisector function. Is used to find the closest year
        // to the mouse position.
        var bisector = d3.bisector(function(d) { return d.year; }).left;
  ...
  _self.draw_chart = function() {
    ...
    // Group to hold the line points.
    // One group for each line points.
    var points = points_group.selectAll(".line-points")
      .data(chart_data);
    // Handle new.
    points.enter().append("g");
    // Remove old.
    ...
    // Update current.
    points
        .attr("class", function(d) { return "line-points " + d.id; });
...
setTimeout(function() {
chart__clean_energy_investments('chartcc');
},10);
